...
export class Translator extends EventEmitter {
    ...
    async stop(kill = false) {
        ...
        await Promise.all(Object.values(this.devices)?.map((dev) => {
    ...
    setupSparkplug() {
        ...
        sp.on('dbirth', (deviceId) => {
            ...
            Object.values(this.devices).find((e) => e._name === deviceId)?._publishDBirth();
        });
        /**
         * What to do when a Sparkplug Birth certificate is requested
         * @param deviceId The Device ID which must produce a birth certificate
        ...
        sp.on('dbirth-all', () => {
            ...
            Object.values(this.devices)?.map((dev) => {
                dev._publishDBirth();
            });
        });
        /**
        ...
        sp.on('dcmd', (deviceId, payload) => {
            ...
            Object.values(this.devices).find((e) => e._name === deviceId)?._handleDCmd(payload);
        });
    ...
    setupConnection(connection) {
        ...
        connection.devices?.forEach((devConf) => {
        ...
        newConn.on('data', (obj, parseVals = true) => {
            connection.devices?.forEach((devConf) => {
    ...
    async fetchConfig(uuid) {
        const cdb = this.fplus.ConfigDB;
        const [config, etag] = await this.retry("config", () => cdb.get_config_with_etag(UUIDs.App.AgentConfig, uuid));
