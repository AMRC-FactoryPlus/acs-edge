...
import { Metrics, sparkplugDataType, } from "./helpers/typeHandler.js";
...
export class SparkplugNode extends (EventEmitter) {
    #conf;
    #client;
    #metrics;
    isOnline;
    ...
    constructor(fplus, conf) {
        ...
        this.#client.on("dcmd", (deviceId, payload) => this.emit("dcmd", deviceId, payload));
        ...
        this.#metrics = new Metrics([
            ...
            ...alertbuilder.build_alert(UUIDs.Alert.ConfigFetchFailed, "Config_Unavailable", this.#conf.alerts?.configFetchFailed ?? false),
            ...alertbuilder.build_alert(UUIDs.Alert.ConfigInvalid, "Config_Invalid", this.#conf.alerts?.configInvalid ?? false),
        ]);
     ...
     */
    async #preparePayload(metrics, birth = false) {
        const newMetrics = [];
        ...
        if (birth)
            payload.uuid = UUIDs.Special.FactoryPlus;
        return payload;
    }
    requestAlias() {
    ...
    async #publishNBirth() {
        ...
        payload.metrics.forEach((metric, index) => {
            this.#metrics.setAlias(index, metric.alias);
        });
     ...
     */
    async publishDBirth(deviceId, metrics) {
    ...
    async #startPublishInterval() {
        ...
        const pubInt = this.#metrics.getByName("Node Control/Publish Interval").value;
    ...
    async #publishMetricBuffer() {
        ...
        await Promise.all(Object.keys(this.#metricBuffer).map(async (deviceId) => {
            ...
            if (metricsToPublish && this.isOnline) {
                ...
                let payload = await this.#preparePayload(this.#metricBuffer[deviceId].splice(0, metricsToPublish));
                await this.#client.publishDeviceData(deviceId, payload, {
                    compress: this.#metrics.getByName("Node Control/Payload Compression").value,
                });
                log(`${metricsToPublish} metrics published from buffer for device ${deviceId}.`);
            }
        }));
    }
    /**
     ...
     */
    async publishDData(deviceId, metrics) {
