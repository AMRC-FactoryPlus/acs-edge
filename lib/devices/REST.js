...
import { restAuthMethod, serialisationType, writeValuesToPayload } from "../helpers/typeHandler.js";
import axios from "axios";
import * as crypto from "crypto";
export class RestConnection extends (DeviceConnection) {
    #baseURL;
    #axiosOptions;
    constructor(type, connDetails) {
        ...
        if (connDetails.authMethod === restAuthMethod.Basic) {
            ...
            const token = Buffer.from(`${connDetails.username}:${crypto
                .createHash("sha256")
                .update(connDetails.password)
     ...
     */
    async writeMetrics(metrics, writeCallback, payloadFormat) {
        await Promise.all(metrics.addresses.map(async (addr) => {
     ...
     */
    async get(address, optionsOverride) {
    ...
    async post(address, data, payloadFormat) {
        // Try to get a response
...
;
export class RestDevice extends (Device) {
    #devConn;
